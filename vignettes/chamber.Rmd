...
`congressbr` makes it easy to explore the API of the Brazilian Chamber of Deputies, or the _Câmara dos Deputados_. The package contains many functions that help scholars to download, standardise and tidy data from the _Câmara_ website. This vignette shows some examples.
...
The function return 19 variables, including the decision situation of the proposition (`desc_situation`), the full text in pdf (`link_complete`), and personal information about the author of the proposition (`name_author`, `abrstate_author`, and `party_author`).
...
Users can download all every bill voted on in plenary in a given calendar year with `cham_plenary_bills()`. The function has a single parameter, `year = `, and returns the id of the bill (`id_bill`), its name (`name_bill`), and the day on which it was voted (`date_voted`)
...
In this brief example, we will examine how the parties orientated their deputies to vote in the law proposition 1992/2007, the same we have downloaded with the `cham_get_votes()` function. This example makes use of the `ggplot2` package.

```{r}
if(!require(ggplot2)){install.packages("ggplot2")}
library(ggplot2)

party_orientation <- cham_get_votes(type = "PL", number = "1992", year = "2007")
...
Similarly, we could also have used the `dplyr` package to see how many parties suggested their members to vote "yay" (_sim_), "nay" (_não_), or allowed deputies to decide their votes (_liberado_).
